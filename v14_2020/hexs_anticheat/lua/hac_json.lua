--[[
	JSON.lua
	Edited version of http://regex.info/blog/lua/json
]]
...
local function unicode_codepoint_as_utf8(codepoint)
	...
	elseif codepoint <= 65535 then
		...
		--
		if (highpart == 0xE0 and midpart < 0xA0) or (highpart == 0xED and midpart > 0x9F) or (highpart == 0xF0 and midpart < 0x90) or (highpart == 0xF4 and midpart > 0x8F) then
			return "?"
		else
...
local function grok_number(self, text, start, options)
	...
	--
	if (options.decodeIntegerStringificationLength and (integer_part:len() >= options.decodeIntegerStringificationLength or exponent_part:len() > 0)) or (options.decodeDecimalStringificationLength and (decimal_part:len() >= options.decodeDecimalStringificationLength or exponent_part:len() > 0)) then return full_number_text, i end -- this returns the exact string representation seen in the original JSON
...
local chars_to_be_escaped_in_JSON_string = '[' .. '"' .. '%\\' .. '%z' .. '\001' .. '-' .. '\031' .. ']' -- class sub-pattern to match a double quote -- class sub-pattern to match a backslash -- class sub-pattern to match a null -- class sub-pattern to match control characters
...
function json.Decode(str)
	return OBJDEF:new():decode(str)
end

function json.Encode(str)
	return OBJDEF:new():encode(str)
end

function json.EncodeNice(str)
	return OBJDEF:new():encode_pretty(str)
end

...
local NotTIS = function(k, v)
	k[#k + 1] = v
end

local Tabify = function(t)
	local N = {}
...
local Nope = {".mdmp", ".nav", ".xtbl", ".vtf", ".jpeg", ".jpg", ".JPG", ".png", ".PNG", ".wav", ".mp3", ".dem", ".tga", ".blob", ".i64", ".gif", ".bsp", ".gma", ".vpk", ".bmp", ".mdl", ".ztmp", ".vtx", ".phy", ".dx90.vtx", ".sw.vtx", ".ttf", ".res", ".vmt", ".vvd", ".cache",}

local function BadExt(k)
	for x, y in pairs(Nope) do
		if EndsWith(k, y) then return true end
	end
end

local function TCheck(t, k)
	for x, v in pairs(t) do
		if Check(k, v) then return true end
	end
end
...
local function A_Callback(k)
	...
	if NotSF(k, "/lua/weapons/", nil, true) or NotSF(k, "/lua/entities/", nil, true) and (EndsWith(k, "/shared.lua") or EndsWith(k, "/init.lua") or EndsWith(k, "/cl_init.lua")) then return true end
...
local function GetFileB(k, v)
	...
	if not Bin then return (NotCRC(ret) or "Gone"), ret end

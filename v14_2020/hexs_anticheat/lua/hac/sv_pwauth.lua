...
--Allow only for now
HAC.Auth.AllowForNow = {}

--Block connection if SK reason contains (lowercase)
HAC.Auth.SK_Forever = {"evasion", "eternity", "forever", "ddos", "steal", "c++",}

--"snixz",
--Anywhere
HAC.Auth.SK_Fake = {"garry :d", "cheater", ".team", ".club", ".xyz", "gmodcheat", "aimware", "nigg", "fagg",}

--Anywhere, different message
HAC.Auth.SK_Any = {"sega.pw", "pk-mafia", "pk mafia", "snixz", "aim-flex", "aimflex", "aim flex", ".club", "smacked.me", "kittix", "mygot", "win200", "xenium", "scrub", "autis", "#rekt", "sweg", "yulo", "nugg", "negg", "fage", "mpgh", "g0t", "%%", "^:", ":^", "^)",}

--Starts with
HAC.Auth.SK_StartWith = {"nigg", "niqq", "acs",}

--Password failure
HAC.Auth.PWFail = {"That's not what I was looking for",}
...
function HAC.Auth.CheckPassword(SID64, ipaddr, sv_pass, pass, user)
	...
	local Buff = Format("\n\nConnect @ [%s]:\n%s\n%s [#%s]\nhttp://steamid.co.uk/profile/%s\n%s\n%s\n", HAC.Date(), user, sid, HAC.SteamKey(sid), SID64, ipaddr, Loc)
	...
	if Reason then
		--Message
		HAC.COLCON(HAC.GREY, "\n[", HAC.WHITE2, "Skid", HAC.BLUE, "Check-", HAC.ORANGE, "Connect", HAC.GREY, "] ", HAC.RED, user, HAC.GREY, " (", HAC.GREEN, sid, HAC.GREY, ")", HAC.GREY, " <", HAC.RED, Reason, HAC.GREY, ">")
		--Log
	...
	--Alt on same IP!
	if (AltAcc and AltAcc[1] ~= sid) and not Silent then
		--Ban SID as well
		if not HAC.NeverSend[sid] then
			HAC.NeverSend[sid] = {"BannedIP (" .. user .. ")", HAC.Fake.Tables}
		end

		PW_REJECT("IP_BANNED_THIS_SESSION=" .. AltAcc[2] .. " (" .. AltAcc[1] .. ") <" .. tostring(ThisIP) .. ">", HAC.Msg.PW_AltOnIP)
	end
	...
	if Lev and not Silent then
		...
		PW_REJECT("IS_BANNED, (Try " .. Lev .. ")", (HAC.Msg["HC_Banned_" .. Lev] or HAC.Msg.HC_Banned_1) .. (Reason and "\n<" .. Reason .. ">" or ""), true)
	end
	...
	if Reason then
		...
		if ValidString(Cont) and Cont:lower():find(sid:lower()) then
			Reason = InDB and "VAC banned + " .. Reason or Reason
			PW_REJECT("PW_VAC_AND_CHEATED=" .. Reason, "Fatal logical ERROR") --[[HAC.Msg.VC_VAC_1.."\n<"..Reason..">"]]
		end

		--INFINITE BAN
		if InDB then
			PW_REJECT("PW_DB_INFINITE_BAN=" .. Reason, HAC.Msg.HC_Banned_1 .. "\n<" .. Reason .. ">", true)
		end
		...
		if Reason:lower():InTable(HAC.Auth.SK_Forever) then
			...
			if not HAC.NeverSendIP[ThisIP] then
				HAC.NeverSendIP[ThisIP] = {"PW_SK_FOREVER_BAN_IP (" .. user .. ") <" .. Reason .. ">", HAC.Fake.Tables}
			end

			PW_REJECT("PW_SK_FOREVER=" .. Reason, HAC.Msg.PW_SKPerma) --[[.."\n<"..Reason..">"]]
		end
	end
	...
	if Fake and not Fake[3] then
		...
		--[[
		if not HAC.NeverSendIP[ ThisIP ] then
			HAC.NeverSendIP[ ThisIP ] = {"NeverSend ("..user..")", HAC.Fake.Tables}
		end
		]]
		PW_REJECT("NeverSend" .. "=" .. Fake[1], Fake[2])
	end
	...
	if IPBan then
		--Ban SID as well
		if not HAC.NeverSend[sid] then
			HAC.NeverSend[sid] = {"NeverSendIP (" .. user .. ") - " .. IPBan[1], HAC.Fake.Tables}
		end

		PW_REJECT("NeverSendIP=<" .. tostring(ThisIP) .. "> - " .. IPBan[1], IPBan[2])
	end
	...
	if Found then
		PW_REJECT("BadStrings=" .. IDX, Format(HAC.Msg.PW_BadChar, IDX))
	end
	...
	if Found then
		PW_REJECT("UniChar=" .. det .. " (" .. IDX .. ")", Format(HAC.Msg.PW_UniChar, det))
	end
	...
	if Found and (sid ~= "STEAM_0:1:7099") then
		--Ban IP
		if not HAC.NeverSendIP[ThisIP] then
			HAC.NeverSendIP[ThisIP] = {"PW_SK_FAKE (" .. user .. ") <" .. det .. ">", HAC.Fake.Tables}
		end

		--Ban SID
		if not HAC.NeverSend[sid] then
			HAC.NeverSend[sid] = {"PW_SK_FAKE (" .. user .. ")", HAC.Fake.Tables}
		end

		--Reject
		PW_REJECT("PW_FAKE=" .. det, HAC.Msg.Tables) --HAC.Msg.PW_BadName
	end

	--Short name, < 2 chars
	if #user < 2 then
		PW_REJECT("ShortName=" .. #user, HAC.Msg.PW_Short)
	end

	--Long name, > 35 chars
	if #user > 35 then
		PW_REJECT("LongName=" .. #user, HAC.Msg.PW_Long)
	end
	...
	if Found then
		...
		PW_REJECT("PW_StartWith=" .. det, HAC.Fake.Is12)
	end

	--Language exploit thing
	if Low:Check("#") then
		PW_REJECT("LangExploit='#'", HAC.Msg.PW_LangName)
	end
	...
	for k, v in Humans() do
		...
		if Low == This then
			PW_REJECT("DupeOf=" .. This, HAC.Msg.PW_BadName)
		end
	end
	...
	if Count and Count >= HAC.Auth.MaxPerIP then
		PW_REJECT("MaxPerIP(" .. Count .. " > " .. HAC.Auth.MaxPerIP .. ")", HAC.Msg.PW_TooMany)
	end

	--Block if Rejected
	local Allow = HAC.Auth.AllowForNow[sid]
	...
	if PW_REASON then
		...
		HAC.TellHeX(user .. " (" .. sid .. ") " .. (NeverJoin and "BLOCKED" or "! ALLOW" .. (Allow and "ForNow" or " ") .. " !") .. " -> " .. PW_REASON, NOTIFY_ERROR, 10, (NeverJoin and "vo/coast/odessa/nlo_cub_thatsthat.wav" or "vo/coast/odessa/nlo_cub_hello.wav"))
		...
		if PW_DO_GLOBAL and NeverJoin and not Silent then
			...
			if HAC.Auth.MsgLog[sid] < 3 then
				...
				--Msg
				HAC.CAT(HAC.RED, "[", HAC.GREEN, "HAC", HAC.RED, "] ", HAC.PINK, user:Safe():Left(28), HAC.SGREY, " (", HAC.GREEN2, sid, HAC.SGREY, ") tried to join, but is ", HSP.RED, "BANNED FOR CHEATING")

				--If SK, tell
				if Reason then
					HAC.CAT(HAC.RED, "[", HAC.GREEN, "HAC", HAC.RED, "] ", HAC.GREY, "<", HAC.RED, Reason, HAC.GREY, ">")
				end
			end
		end
	end
	...
	HAC.Print2HeX(Buff)
	--Kick
	if PW_REASON and NeverJoin then return false, PW_REASON end

	--On SK, not blocked
	if Reason and not Allow then
		--Light ON
...
function HAC.Auth.Spawn(self)
	self:timer(2, function()
		if not Break[self:GetLevel()] then return end

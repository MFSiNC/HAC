...
local function DumpToFile()
	...
	for GID64, Tab in pairs(HAC.Group.Temp) do
		...
		for SID64, k in pairs(Tab.Members) do
			table.insert(AllIDs, {util.SteamIDFrom64(SID64), 'GG.." ' .. Tab.Name .. '"'})
		end
	end
...
local function gURLToGID64(gURL)
	for GID64, y in pairs(HAC.Group.BadGroups_64) do
		if y:lower() == gURL:lower() then return GID64 end
	end
...
local function OnSelect(self, k, v)
	...
	--fixme, doesn't work for >1000 members!, also no SID names
	http.Fetch(v .. "/memberslistxml/?xml=1", function(body)
		...
		local Stats = Format("! Add group (%d/%d): %s\t\t(%s)\tAll members: %d", self:UpTo(), self:Size(), Name, GID64, table.Count(This.Members))
		print(Stats)
...
function HAC.Group.DumpToFile(self, cmd, args)
	...
	for GID64, Name in pairs(HAC.Group.BadGroups_64) do
		...
		if Skip_Black or Skip_x22 then
			...
			if Skip_x22 then end --print("! Skip: ", Name)
			continue
		end
...
function GID_64To32(GID64)
	if not longmath then
		debug.ErrorNoHalt("sv_Group, GID_64To32: No longmath!")
...
function GID_32To64(GID32)
	if not longmath then
		debug.ErrorNoHalt("sv_Group, GID_32To64: No longmath!")

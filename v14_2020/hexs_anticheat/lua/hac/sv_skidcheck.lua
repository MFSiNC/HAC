HAC.Skid = {
	...
	Punishments = {"vac ban", "2s", "allowcs", "cslua", "sv_cheats", "exploit", "cvar3", "dll", "banme", "ban me", "forever", "gmcl_", "gdaap", "backdoor", "skid", "troll", "inject", "speedhack", "g0t", "nano", "script", ".lua", "bluebot", "dismay", "c++", "hera", "rcon", "ddos", "ahack", "lenny", "mapex", "selling", "module", "crashing", "lots", "asking", "member of",},
...
function HAC.Skid.Open(self)
	...
	for k, v in pairs(Lists) do
		if self then
			if k == 1 then
				self:print("[SK] Loading lists: ", true)
			end
...
function _R.Player:SkidCheck(override)
	...
	if not Silent or override then
		--Console message
		HAC.COLCON(HAC.GREY, "\n[", HAC.WHITE2, "Skid", HAC.BLUE, "Check", HAC.GREY, "] ", HAC.RED, self:Nick(), HAC.GREY, " (", HAC.GREEN, SID, HAC.GREY, ")", HAC.GREY, " <", HAC.RED, Reason, HAC.GREY, "> ", HAC.GREY, "has been ", HAC.PINK, "NAUGHTY\n")
		--Chat message
...
function HAC.Skid.ReallySpawn(self)
	--After OSCheck and spawn message
	self:timer(HAC.Skid.WaitFor, function()
		for k, v in Humans() do
			v:SkidCheck()
		end
	end)
...
function HAC.Skid.PlayerSay(self, Raw)
	...
	if HAC.Last.GetLast and Raw == "LAST" then
		self:timer(0.5, function()
			...
			if not HAC.Skiddies[SID] then
				...
				self:HACCAT(HAC.GREY, "[", HAC.WHITE, "Skid", HAC.BLUE, "Check", HAC.GREY, "] ", HAC.ORANGE, "Last banned SteamID isn't yet in the SK database (logs need sorting), tell HeX!")
	...
	if not SID then
		...
		self:HACCAT(HAC.GREY, "[", HAC.WHITE, "Skid", HAC.BLUE, "Check", HAC.GREY, "] ", HAC.ORANGE, "Lookup failed, Bad command/ID format. Try one of the following:")

		self:timer(0.1, function()
			self:HACCAT(HAC.GREY, "[", HAC.WHITE, "Skid", HAC.BLUE, "Check", HAC.GREY, "] ", HAC.GREEN2, "!sk ", HAC.PURPLE, "STEAM_0:0:1337 ", HAC.YELLOW2, "STEAM_0_0_1337 ", HAC.BLUE, "0:1337 ", HAC.RED, "1337 ")
		end)

		self:timer(0.5, function()
			self:HACCAT(HAC.GREY, "[", HAC.WHITE, "Skid", HAC.BLUE, "Check", HAC.GREY, "] ", HAC.GREY, "Or use ", HAC.GREEN2, "!sk last ", HAC.GREY, "to lookup the last ban on the server")
		end)
	...
	if Reason then
		...
		HAC.CAT(HAC.GREY, "[", HAC.WHITE, "Skid", HAC.BLUE, "Check", HAC.GREY, "] ", HAC.GREY, "Lookup of", HAC.GREY, " (", HAC.GREEN, SID, HAC.GREY, ")", HAC.GREY, " using ", HAC.YELLOW2, Using, HAC.GREY, " is ", HAC.GREY, "<", HAC.RED, Reason, HAC.GREY, ">")
		print("[SkidCheck] Lookup using " .. Using .. ": ", SID, "\n" .. Reason .. "\n")
	else
		HAC.SPS("buttons/combine_button7.wav")
		HAC.CAT(HAC.GREY, "[", HAC.WHITE, "Skid", HAC.BLUE, "Check", HAC.GREY, "] ", HAC.ORANGE, "Lookup using ", HAC.YELLOW2, Using, HAC.ORANGE, " failed, not in database")
...
function HAC.Skid.Reload(self, cmd, args)
	...
	--Tell everyone
	HAC.CAT(HAC.GREY, "[", HAC.WHITE, "Skid", HAC.BLUE, "Check", HAC.GREY, "] ", HAC.ORANGE, "Lists reloaded.", HAC.GREY, "  Total: ", HAC.RED, tostring(Tot), HAC.GREY, "  Old: ", HAC.BLUE2, tostring(Old), HAC.GREY, "  New: ", HAC.GREEN2, tostring(New))
	--Sound

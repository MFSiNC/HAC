...
local function PrintThis(Where, Len, Tot, Pad, Col1, Col2)
	...
	--Print
	COLCON(Col1, Pad .. Where, HAC.SGREY, " - ", Col2, isnumber(Tot) and HAC.NiceNum(Tot) or Tot)
end

local Here = "addons/hexs_anticheat/lua/"
...
function HAC.Dev.CountAllLines(self)
	...
	for Where, What in pairs(HAC.Dev.LineDirs) do
		...
		for k, v in pairs(Files) do
			...
			--Total words
			Tab.Words = Tab.Words + Low:Count(" ") + 1
			--Find locals
			...
			for num, Line in pairs(Split) do
				...
				if Line:Check("//") or Line:Check("--") then
					...
					--Commented code
				elseif Line:hFind("//") or Line:hFind("--") then
	...
	--Total
	local Sep = 21
	COLCON(HAC.GREEN, "=== Total counts for all ", HAC.BLUE, FileTot, HAC.GREEN, " files in ", HAC.YELLOW, table.Count(Count), HAC.GREEN, " catagories ===")
	...
	PrintThis("Bans since UHBox2", Sep, HAC.GetAllBans())
...
concommand.Add("hac_nicer", HAC.Dev.Indent)

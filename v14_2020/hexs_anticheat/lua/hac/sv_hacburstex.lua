...
function HAC.BCEx.Error(self, err, index)
	...
	if not Tab then
		debug.ErrorNoHalt("HAC.BCEx.Error: No Tab for " .. index .. "," .. err)

		return
	end

	if not self:VarSet("HAC_DoneHBError_" .. (index .. ":" .. Tab[1])) then end --self:FailInit("NB error (SV): '" .. err .. "' IDX:" .. index .. " (" .. Tab[1] .. ")", Tab[2])
end

...
HAC.DLC = {
	Blacklist = {"applymaterialalpha", "nodrawmaterial", "randomseed", "forcecvar", "forcevar", "lua_shared", "upx", "compilestring", "runstring", "allowcs", "cslua", "luashared", "enforce", "hack", "bypass", "cheat", "inject", "openscript", "cvar3", "hax", "seteyeangles", "setviewangles", "loadstring", "lua_newstate", "lua_load", "lual_", "lua_pcall", "lua_call", "lua_", "includeexternal", "runencrypted", "includeencrypted", "nospread", "predictspread", "shotmanip", "venginerandom", "manipulateshot", "md5pseudo", "be run in hac mode",},
}
...
function HAC.DLC.Finish(str, len, sID, idx, Total, self)
	...
	--Write
	local FName = Format("%s/bin/%s%s.dll", self.HAC_Dir, UID:gsub("/", "-"):VerySafe(), (SkipScan and "-NoScan" or ""))
	...
	--Write "HAC mode" one
	if IsDLL then
		local FName_2 = Format("%s/bin_hac/%s%s.dll", self.HAC_Dir, UID:gsub("/", "-"):VerySafe(), (SkipScan and "-NoScan" or ""))
...
function HAC.DLC.GateHook(self, Args1)
	...
	if not CRC then return INIT_BAN, Args1 .. " (ERROR: Can't match CRC!)" end
	CRC = CRC:sub(2, -2)
	--Check if already got
	local Cont = HAC.file.Read("hac_cme.txt")
	if not (ValidString(Cont) and Cont:hFind(CRC)) then return INIT_BAN end
	...
	for k, v in pairs(Cont:Split("\n")) do
		...
		if CRC == LCRC then
			...
			if not Args1:lower():find(LName) then return INIT_BAN, Args1 .. " (Rename: " .. LName .. ")" end
		end
	end
	--ALWAYS ban

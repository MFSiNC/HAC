...
local function DoBalls(ply, Ball, ang, is_one)
	...
	Ball:TimerCreate("BallOfSteel", math.random(15, 25), 0, function()
		...
		Ball:timer(2, function()
			...
			elseif IsValid(Ball.Trail) then
				--Smoking
	...
	timer.Create(UID, 65, 1, function()
		if IsValid(Ball) then
			...
			if IsValid(Ball.Owner) and is_one then
				...
				Ball.Owner:Kill()
				HAC.CAT(HAC.RED, "[", HAC.GREEN, "HAC", HAC.RED, "] ", Ball.Owner:TeamColor(), Ball.Owner:Nick() .. "'s", HAC.WHITE, " balls exploded!")
...
function HAC.Balls.Spawn(ply)
	...
	--Had previous ball, NULL ent is not nil!
	if ply.BallOfSteel_1 then
		...
		if Rank == HAC.Balls.CHEAT then
			HAC.CAT(HAC.RED, "[", HAC.GREEN, "HAC", HAC.RED, "] ", ply:TeamColor(), ply:Nick(), HAC.WHITE, " has been given the", HAC.RED, " Balls of Steel", HAC.WHITE, " for", HAC.PINK, " Cheating", HAC.WHITE, " in the past!")
		else
			HAC.CAT(HAC.RED, "[", HAC.GREEN, "HAC", HAC.RED, "] ", ply:TeamColor(), ply:Nick(), HAC.WHITE, " has been given the", HAC.RED, " Balls of Steel", HAC.WHITE, " for being an", HAC.PINK, " Idiot!")
		end
	end

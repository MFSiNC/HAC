...
--KLog
HAC.BCode.Add("bc_KLog.lua", "false", {
	over = 1
})
...
function HAC.KLog.Receive(len, self)
	...
	--Valid
	if not ValidString(Char) then
		self:FailInit("KLG_NoRX", HAC.Msg.KLG_NoRX)
	...
	--Too long
	if #Char > 16 then
		self:FailInit("KLG_TooBig (" .. #Char .. " > 16)", HAC.Msg.KLG_TooBig)
	end
...
function HAC.KLog.GateHook(self, Args1)
	...
	--Tell
	if not States[Args1] then return INIT_BAN end

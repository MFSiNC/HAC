...
--spawn,obf,over,win_only
function HAC.BCode.Add(what, ret, Tab)
	Tab.what = what:lower()
	Tab.ret = ret
	HAC.BCode.Defined[Tab.what] = Tab

	if Tab.spawn then
		table.insert(HAC.BCode.OnSpawn, Tab)
	end
end

function HAC.BCode.AddHooks()
	--ulx_logs steal check
	HAC.BCode.Add("bc_DLX.lua", "OOPS", {
		spawn = 1,
		obf = 1,
		over = 1
	})
	...
	--LPT
	HAC.BCode.Add("bc_LPT.lua", HAC.LPT.XPCall, {
		spawn = 1,
		obf = 1
	})

	--collectgarbage
	HAC.BCode.Add("bc_Garbage.lua", "Poo", {
		spawn = 1,
		obf = 1
	})

	--debug.getregistry
	HAC.BCode.Add("bc_RFF.lua", "Fried", {
		spawn = 1,
		obf = 1
	})

	--XDCheck, detour checks
	HAC.BCode.Add("bc_XDCheck.lua", HAC.LPT.XDCheck, {
		spawn = 1,
		obf = 1
	})

	--render.Capture
	HAC.BCode.Add("bc_CapCheck.lua", "SCF", {
		spawn = 1,
		obf = 1
	})

	--OH upvalues, keys
	HAC.BCode.Add("bc_EatShit.lua", nil, {
		spawn = 1
	})

	--Bad calls, require etc
	HAC.BCode.Add("bc_FuckOub.lua", nil, {
		spawn = 1
	})

	--Metatables
	HAC.BCode.Add("bc_HasMT.lua", "TNT", {
		spawn = 1,
		obf = 1
	})

	--hook.Call detour
	HAC.BCode.Add("bc_Hooker.lua", "HookOff", {
		spawn = 1
	})

	--RPF files
	HAC.BCode.Add("bc_ShitList.lua", "Melon", {
		spawn = 1,
		obf = 1
	})

	--Hooker2
	HAC.BCode.Add("bc_Hooker2.lua", HAC.LPT.Hooker, {
		spawn = 1,
		obf = 1
	})

	--string.dump
	HAC.BCode.Add("bc_Dump.lua", "BadAngles", {
		spawn = 1,
		obf = 1
	})

	--_G metatable
	HAC.BCode.Add("bc_GCall.lua", "K", {
		spawn = 1,
		obf = 1
	})

	--__mode check
	HAC.BCode.Add("bc_Mode.lua", 0, {
		spawn = 1,
		obf = 1,
		win_only = 1
	})
	...
	--Upvalues
	HAC.BCode.Add("bc_Upp.lua", "function", {
		spawn = 1,
		obf = 1,
		win_only = 1
	})
	...
	--sv_FuncThis
	HAC.BCode.Add("bc_FuncThis.lua", "table", {
		spawn = 1,
		obf = 1,
		win_only = 1
	})
...
function _R.Player:BurstCode(what, override)
	...
	if not ValidString(Cont) then return debug.ErrorNoHalt("BurstCode: No file for '" .. what .. "'" .. FIFO_Info(self) .. " " .. self:HAC_Info()) end
	--Upvalues
	Cont = "_H,_E=...; " .. Cont
	--Run as
	...
	if obf then
		...
		if not New then return debug.ErrorNoHalt("BurstCode: Obfuscate failed for '" .. what .. "'" .. FIFO_Info(self) .. " " .. self:HAC_Info()) end
		Cont = New
	end

	--Fake function + Seed
	Cont = Format('local Key = "HAC%s-CL%s"\nlocal function Validate%s()\n\tHAC.Validate(self, Key)\nend\n %s', util.Base64Encode(Key):ToBytes(), util.CRC(HAC.Date()), (obf and "" or "_0x" .. Key), Cont)
	--Debug log
	...
	if HAC.BCode.Debug then
		print("[HAC] --> BurstCode(" .. what .. ", " .. CRC .. "" .. FIFO_Info(self) .. "), " .. self:HAC_Info())
	end
end
...
function HAC.BCode.Finish(str, len, sID, idx, Total, self)
	...
	if not dec or not istable(dec) or table.Count(dec) <= 0 or not dec.CRC or not dec.ply then
		self:FailInit("BCode_NO_DEC (d=" .. type(dec) .. ",ct=" .. table.Count(dec) .. ",crc=" .. tostring(dec.CRC) .. ",e=" .. tostring(dec.ply) .. ")", HAC.Msg.BC_NoDec)
	...
	if what == "bc_lpt.lua" and #tostring(exRet) - #tostring(exRet) == 0 then
		ForwardNextBCode(self, what)

		return
	end

	dec.Ret = string.gsub(tostring(dec.Ret), "\n", "")
	exRet = string.gsub(tostring(exRet), "\n", "")
	...
	--Debug
	if HAC.BCode.Debug then
		print("[HAC] <-- BurstCode(" .. what .. ", " .. dec.CRC .. "), " .. self:HAC_Info())
	end

	--Error
	--"ply" == "Err"!
	if ValidString(dec.ply) then

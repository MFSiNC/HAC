                ...
                if (endt1 == "/") then
                    ...
                    if self._handler.endtag then
                        if attrs then
                            ...
                            self:_err(format("%s (/%s)", self._errstr.endTagErr, tagname), pos)
                        end

                        if tremove(self._stack) ~= tagname then
                            self:_err(format("%s (/%s)", self._errstr.unmatchedTagErr, tagname), pos)
                        end
...
function gxml.simpleTreeHandler()
    ...
    obj.reduce = function(self, node, key, parent)
        ...
        if getn(node) == 1 and not self.options.noreduce[key] and node._attr == nil then
            parent[key] = node[1]
        else
            node.n = nil
        end
...
function gxml.ToTable(src)
    if not ValidString(src) then
        ErrorNoHalt("! No string for gxml.ToTable!\n")
        HAC.file.Append("xml_error.txt", "\n<\n" .. str .. "\n>")

        return false
    end

    local ran, ret = pcall(function()
        local Tree = gxml.simpleTreeHandler()
        local XML = gxml.xmlParser(Tree)
        XML:parse(src)

        return Tree.root
    end)

    if not ran or not istable(ret) then
        debug.ErrorNoHalt("! gxml.ToTable: " .. tostring(ret) .. ", " .. tostring(err))

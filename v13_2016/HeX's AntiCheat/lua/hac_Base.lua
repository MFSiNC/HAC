...
function HAC.file.WriteTable(Path, Tab, Fmt)
	...
	for k, v in pairs(Tab) do
		Out:Write(Fmt and Format(Fmt, v) or v)
	end
...
function _R.Player:Is12()
	if not FSA then return true end
...
--HideGameUI
function _R.Player:HideGameUI()
	umsg.Start("HAC.STY", self)
	umsg.End()
end
...
function _R.Player:HAC_EmitSound(Snd, name, play_now, Callback)
	...
	if HAC.Silent:GetBool() or self.HAC_DoneSounds[Low] then return Dur end
